npm-install(1) -- Install a package 安装一个库
===================================
...
此命令安装一个库及此库依赖的任何其他库。如果这个库有package-lock或者shrinkwrap文件，则安装过程由这个文件驱动，如果两个文件都存在，npm-shrinkwrap.json会优先应用。查看package-lock.json和npm-shrinkwrap了解更多。
...
* a) 一个目录包含着一个由package.json文件描述的程序库清单
* b) 一个gzip格式的档案，包含了(a)
* c) 指向(b)的url
* d) 一个<name>@<version>格式的含(c)的发布到registry的库
* e) 
* f) 
* g) 
